...
public class GameBoard {
    ...
    public final Cell[][] board = new Cell[NUMBER_ROWS][NUMBER_COLS];
    ...
    public String[][] stringCellStates(boolean gameOver, boolean cheating) {
        String[][] cellState = new String[NUMBER_ROWS][NUMBER_COLS];
        for (int row = 0; row < NUMBER_ROWS; row++) {
            for (int col = 0; col < NUMBER_COLS; col++) {
                cellState[row][col] = board[row][col].stringState(gameOver, cheating);
            }
        }
        return cellState;
    }
